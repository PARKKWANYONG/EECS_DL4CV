...
Original file is located at
    https://colab.research.google.com/drive/1pkBjNJ3uVcwHlrC2x6Tl4FwJ7MmOqELn
"""
...
Original file is located at
    https://colab.research.google.com/drive/1-MUROrNvcy0vtP-5K2oZzSuDpultrwFN
"""
...
def nn_forward_pass(params, X):
    ...
    scores. The network architecture should be:                                                 forward 해서 score hidden value
    ...
    hidden = X.mm(W1) + b1                                                                                              # hidden                             
    pre_relu = hidden.clone()                                                                                           # relu 전 hidden을 복사
    hidden[hidden < 0] = 0                                                                                              # relu 후 hidden
    scores = hidden.mm(W2) + b2                                                                                         # score
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    shifted_logits = scores - scores.max(dim=1, keepdim=True).values                  # 확률로 바꾸기 위함                                   
    Z = shifted_logits.exp().sum(dim=1, keepdim=True)                                 # 확률로 바꾸기 위함
    log_probs = shifted_logits - Z.log()                                              # 확률로바꿈
    probs = log_probs.exp()                                                           # backpropa 계산을 위하여 probs를 따로 구해준다.
    loss = (-1.0/ N) * log_probs[torch.arange(N), y].sum()                            # 배치사이즈 나누고 -logP(c) (각 배치에서 올바른 Class의 확률에 log를 씌운 값)
    loss += reg * torch.sum(W1 * W1) +  reg * torch.sum(W2 * W2)                      # 총 loss 
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    d_scores[torch.arange(N), y] -= 1                                               #아까 올바른 class는 - x가 더붙는다
    d_scores /= N

    grads['b2'] = d_scores.sum(dim = 0)
    grads['W2'] = h1.t().mm(d_scores) + 2* W2 * reg                                 # 이전에 구했던 softmax의 dw vectorize를 그대로 사용. h1가 x라고 생각.
  
    d_h1 = d_scores.mm(W2.T)                                                        # 모르겠음.
    d_h1[h1 == 0] = 0
    grads['b1'] = d_h1.sum(dim = 0)
    grads['W1'] = X.t().mm(d_h1) + 2* W1 * reg                                      #이전에 구했던 
...
def nn_train(params, loss_func, pred_func, X, y, X_val, y_val,
  ...
  Inputs:
  - params: a dictionary of PyTorch Tensor that store the weights of a model.                           가중치를 Train 시키
    It should have following keys with shape
        W1: First layer weights; has shape (D, H)
        b1: First layer biases; has shape (H,)
        W2: Second layer weights; has shape (H, C)
  ...
  for it in range(num_iters):
    X_batch, y_batch = sample_batch(X, y, num_train, batch_size)                        # X배치 Y배치

    # Compute loss and gradients using the current minibatch
    loss, grads = loss_func(params, X_batch, y=y_batch, reg=reg)                        # loss_func에서 loss와 gradient
    loss_history.append(loss.item())                                                    # loss를 빈 리스트에추가
    ...
    #########################################################################
    # Replace "pass" statement with your code
    params['W1']-= grads['W1']*learning_rate                                                    #가중치를 업데이트 
    ...
    if verbose and it % 100 == 0:                                                               # 이터레이션 100회마다 아래값들 반환
      print('iteration %d / %d: loss %f' % (it, num_iters, loss.item()))

    # Every epoch, check train and val accuracy and decay learning rate.
    if it % iterations_per_epoch == 0:                                                           # epoch마다 아래 수행
      # Check accuracy
      y_train_pred = pred_func(params, loss_func, X_batch)                                      #예측                         
      train_acc = (y_train_pred == y_batch).float().mean().item()                               #train_정확도
      y_val_pred = pred_func(params, loss_func, X_val)                                          # validation 예측
      val_acc = (y_val_pred == y_val).float().mean().item()                                     #validation 정확도
      train_acc_history.append(train_acc)                                                       #빈리스트에추가
      val_acc_history.append(val_acc)                                                           #빈리스트추가

      # Decay learning rate
      learning_rate *= learning_rate_decay                                                      #weight decacy
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  results,_ = nn_forward_pass(params, X)                                                                #forward_pass 는 scores, hidden
  _, y_pred =  results.max(dim = 1)                                                                     # score중 최대값 index
  pass
  ###########################################################################
  #                              END OF YOUR CODE                           #
  ###########################################################################
...
def nn_get_search_params():
  """
  Return candidate hyperparameters for a TwoLayerNet model.                                                          조합 256 이하 parmeter들 세팅 
  ...
  for reg in regularization_strengths:
    for lr in learning_rates:
      for hs in hidden_sizes:
        ...
        net = TwoLayerNet(3 * 32 * 32, hs, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)                                # 모델지정
        stats = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],                                       #가중치 train
                  num_iters=3000, batch_size=1000,
                  learning_rate=lr, learning_rate_decay=0.95,
                  reg=reg, verbose=False)
        if max(stats['val_acc_history']) > best_val_acc:                                                                                            # 리턴 값     'loss_history': loss_history,
                                                                                                                                                    #        'train_acc_history': train_acc_history,
                                                                                                                                                    #              'val_acc_history': val_acc_history,
          best_val_acc = max(stats['val_acc_history'])
          best_net = net                                                                                                                            #베스트 모델지정
          best_stat = stats                                                                                                                         # 베스트 train상태 
  pass
  #############################################################################
  #                               END OF YOUR CODE                            #
  #############################################################################
