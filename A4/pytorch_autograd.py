...
Original file is located at
    https://colab.research.google.com/drive/1PouFZhG8AvOfDkkvYnEG1DhikwlBZFz8
"""
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  conv_w1 = nn.init.kaiming_normal_(torch.empty(channel_1, C,kernel_size_1,kernel_size_1, dtype=dtype, device=device))    # conv1 weight 초기화
  conv_w1.requires_grad = True
  conv_b1 = nn.init.zeros_(torch.empty(channel_1, dtype=dtype, device=device))  # bias
  conv_b1.requires_grad = True
  H = (H - kernel_size_1 + 2*2)/1 +1    # 새로운 H
  W = (W - kernel_size_1 + 2*2)/1 +1    # 새로운 W
  ...
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes,channel_2*H*W, dtype=dtype, device=device))    #FC LAYER 초기화
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    stride = 2 if downsample  else 1                                  # STRIDE downsample이면 2 아니면 1 (크기)
    Cmiddle = int((Cin+ Cout)/2)
    self.net = nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(Cin,Cmiddle, 3,padding = 1,stride = stride),          # 가운데 블락 
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if (Cin==Cout) and not downsample:                                      # 같다면 그대로
      self.shortcut = nn.Identity()
    elif (Cin != Cout) and not downsample:                                  # 입력채널 출력채널 같지않다면
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    for arg in  stage_args:
      blocks.append( ResNetStage(*arg))                                               #block추가
    blocks.append(nn.AvgPool2d(8))                                                     # Avgpooling추가 
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(Cin,C_4, 1,stride = stride),                                  # bottle neck
      nn.BatchNorm2d(C_4),
      nn.ReLU(),
