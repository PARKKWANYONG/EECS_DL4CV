# -*- coding: utf-8 -*-
"""netwokr_visualization.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1YDtygikbgf8vDA6ZAju484HGYtbGj5Dv
"""
...
def compute_saliency_maps(X, y, model):
  ...
  res = model(X)
  correct_scores = res[range(X.shape[0]),y]                  #scroe를 저장하고 
  correct_scores.sum().backward()                             #gradient가 나오게 backword
  saliency, _  = X.grad.max(dim = 1)                        # 각 부분에서 grad가 가장큰부분 
  ##############################################################################
  #               END OF YOUR CODE                                             #
  ##############################################################################
  return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  learning_rate = 1
  ##############################################################################          . adversarial attack도 조건에 따라서 만들어주면 됨
  ...
  for i in range(max_iter):
    print(i)
    res = model(X_adv)                                                                            #위에 gradient 공식 나와있는데로 써주고 training loop로 만들어줌
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # after each step.                                                     #
    ########################################################################                      # score를 maximize하는 것을 시각화
    # Replace "pass" statement with your code
    img = img.requires_grad_()
    res = model(img)                                                                              # 모델에 이미지 넣고 조건대로 l2 규제 써서 backward
    correct_score = res[:, target_y] - l2_reg*(img*img).sum()
    correct_score.backward()
    #print(img.data)
    img.data =  img +img.grad*learning_rate                                                        # 시각화
    img.grad.zero_()
    #print(correct_score)
    ########################################################################
    #                             END OF YOUR CODE                         #
