...
Original file is located at
    https://colab.research.google.com/drive/1AEj9V135WYccPfFo1G5wAvSZ6dm7b3EW
"""
...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # print(Wx.shape)
    xW = x.mm(Wx)                                                                       #인풋과 가중치곱
    prev_hH = prev_h.mm(Wh)+b                                                           # 이전 h * 가중치 + bias
    for_tanh = xW + prev_hH                                                             # 하이퍼볼릭전
    next_h = torch.tanh(for_tanh)                                                       # 하이퍼볼릭 후 h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape                                                             # N배치 , T (시퀀스?), D 디멘
    _ , H = h0.shape                              
    h= torch.zeros(N,T,H,**to_double_cuda)
    prev_h = h0
    cache = []
    for t in range(T):                                                             # 시퀀스만큼
      prev_h, local_cache = rnn_step_forward(x[:,t], prev_h, Wx, Wh, b)            # RNN 거침 순차적
      h[:,t] = prev_h
...
class WordEmbedding(nn.Module):
  ...
  def forward(self, x):
      ...
      ##############################################################################
      # Replace "pass" statement with your code
      out = self.W_embed[x]                                                             # input 값 임베딩 
      ##############################################################################
      #                               END OF YOUR CODE                             #
      ##############################################################################
      return out
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # print(x_new.shape)
    # print(y_new.shape)
    loss = torch.nn.functional.cross_entropy(x_new,y_new,ignore_index =ignore_index,reduction ='sum')/x.shape[0]      # crossentropy loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.feature_extractor = FeatureExtractor(pooling=True, verbose=False,device=device, dtype=dtype)       # 특징추출
        if cell_type == 'attention':
          self.feature_extractor_A = FeatureExtractor(pooling=False, verbose=False,device=device, dtype=dtype)  # ATTENTION
          self.conv1 = torch.nn.Conv2d(1280, hidden_dim,(1,1)).to( device=device, dtype=dtype)
        self.Linear1 = nn.Linear(1280,hidden_dim).to(device = device, dtype = dtype)                # Linear
        self.WE = WordEmbedding(vocab_size, wordvec_dim,device = device, dtype = dtype )            #임베딩
        if cell_type == 'rnn':                                                                      #RNN이면 
          self.RNN = RNN(wordvec_dim,hidden_dim,device = device, dtype = dtype)
        elif cell_type == 'lstm':                                                                   #LSTM이면
        ...
        self.Linear_hidden_scores =  nn.Linear(hidden_dim, vocab_size).to(device = device, dtype = dtype)     # 끝 단 HIDDEN SCORE 
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def forward(self, images, captions):
        ...
        captions_in = self.WE(captions_in)                                          #임베딩거친거
        h_states = self.RNN(captions_in, images).to(dtype = self.dtype )            # RNN에 CAPTION과 특징출된 images를 넣어 히든스테이트값 반환
        h_states = self.Linear_hidden_scores(h_states)                              # 아웃풋ㄱ값 스코어
        loss = temporal_softmax_loss(h_states,captions_out, ignore_index=self._null)    #loss계산
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b, attn=None, Wattn=None):
    ...
    if attn != None:
      # print(attn.shape)
      # print(Wattn.shape)
      WXattn = attn.mm(Wattn)             # attentionvalue ㅇ
    else:
      WXattn = 0                          # attnvalue는 x
    H = prev_h.shape[1]
    WxX = x.mm(Wx)
    WhH = prev_h.mm(Wh)
    a = WxX+WhH+b + WXattn
    ...
    next_c = f*prev_c + i*g               #정보 얼마나저장
    next_h = o*torch.tanh(next_c)         # 다음 히든스테이트
...
def dot_product_attention(prev_h, A):
    ...
    Matt = (torch.bmm(h_tilt, A_tilt).div(sqrt(H))).reshape(N, -1, 1)             # h와 A의 attention score
    Matt_tilt = F.softmax(Matt, dim=1)                                            # attention distribution
    # print(Matt_tilt.shape)
    attn = torch.bmm(A_tilt, Matt_tilt).reshape(N, H)                             # attention value 
    # print(attn.shape)
...
def attention_forward(x, A, Wx, Wh, Wattn, b):
    ...
    prev_h = h0
    prev_c = c0
    attn, attn_weights = dot_product_attention(prev_h,A)                            #위에서 ㅇ
    for t in range(T):                                                              #순차적연산
